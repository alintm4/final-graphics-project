...
async def main():
    FOV = 60
    pg.init()
    pg.display.set_caption("जोम्बि मेज!")
    ...
    pg.mouse.set_visible(True)  # Show cursor initially
    pg.mouse.set_pos(400,300)   # Center mouse position
...
def gen_map(size):
    mapc = np.random.randint(0,255, (size,size,3)) 
...
def get_sprites(level):
    sheet = pg.image.load('Assets/Sprites/zombie_n_skeleton'+str(level)+'.png')
    sprites = [[], []]
    swordsheet = pg.image.load('Assets/Sprites/gun1.png')
    sword = []
    for i in range(3):
